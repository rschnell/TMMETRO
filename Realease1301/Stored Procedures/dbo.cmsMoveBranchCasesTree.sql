...
CREATE PROCEDURE [dbo].[cmsMoveBranchCasesTree] 
(
	@CategoryID integer,
	@SourceTreeID integer,
	@TargetTreeID integer
...
IF LEN(@SourceTreeID) = 0
BEGIN
	SET @Error = 113 -- ERROR: NodeNamee required
END

IF LEN(@TargetTreeID) = 0
BEGIN
...
SET @LNS=(SELECT TREELN AS LNS
            FROM  CASESTREE
            WHERE TREEID = @SOURCETREEID AND CATEGORYID=@CATEGORYID)
...
SET @LNT=(SELECT TREELN AS LNT
            FROM  CASESTREE
            WHERE TREEID = @TARGETTREEID  AND CATEGORYID=@CATEGORYID)
