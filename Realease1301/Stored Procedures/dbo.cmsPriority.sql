...
CREATE PROCEDURE [dbo].[cmsPriority] 
(
	@CaseID		integer,
	@ActionID	integer = 0,
	@UserID 	integer = 0,
...
BEGIN
	...
	BEGIN
		...
		IF @MgrID <> @UserID
		BEGIN
			SET @Error = 104 -- ERROR: Cannot change priority. User not case manager
		END
	END
	ELSE
	BEGIN
		...
		IF @MgrID <> @UserID
		BEGIN
			SET @Error = 105 -- ERROR: Cannot change priority. User not action initiator
		END
	END
END
...
BEGIN
	IF @ActionID = 0
	BEGIN
		UPDATE CASES SET	CASES._CMSPriorityOf = @Priority,
							CASES._CMSDateUpdated = @Date
					WHERE	CASES.CaseID = @CaseID
	END
	ELSE
	BEGIN
		UPDATE ACTIONS SET	ACTIONS._CMSPriorityOf = @Priority,
		                    ACTIONS._CMSRiskOf = @Risk,
