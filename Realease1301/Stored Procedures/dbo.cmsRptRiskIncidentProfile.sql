...
CREATE PROCEDURE dbo.cmsRptRiskIncidentProfile
...
where (T.NODETAG in ('ARS','BLM','BMB','BRG','COR','CFM','FRD','HLD','IS','KDN','TFT','OTH'))

-- aggregate for category and period
select PERIOD,
       CATEGORY,
       case CATEGORY when 'ARS' then 'Arson'
                     when 'BLM' then 'Blackmail'
                     when 'BMB' then 'Bomb Threat'
                     when 'BRG' then 'Burglary'
                     when 'COR' then 'Corruption'
                     when 'CFM' then 'Counterfeit'
                     when 'FRD' then 'Fraud'
                     when 'HLD' then 'Holdup'
                     when 'IS'  then 'Information Security'
                     when 'KDN' then 'Kidnap'
                     when 'TFT' then 'Theft'
                     when 'OTH' then 'Other' end as CATEGORYNAME, 
